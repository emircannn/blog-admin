...
const EditCon: React.FC<Props> = ({
  ...
  const [_data, setData] = useState<Magazine[]>([])
  ...
  const handleSubmit = async() => {
    try {
        ...
        if(form.file || form.title || form.startPage || form.magazineId || form.endPage) {
            ...
            form.endPage && formData.append('endPage', form.endPage)
            form.startPage && formData.append('startPage', form.startPage)
            form.title && formData.append('title', form.title)
            form.magazineId && formData.append('magazineId', form.magazineId)
            form.file && formData.append('filePdf', form.file)
            const res = await axios.post(`${process.env.NEXT_PUBLIC_URL}contribution/update?id=${data.id}`, formData,{
            headers: {
        ...
        <DialogContent className="w-[500px]">
            ...
            <div className="flex flex-col gap-4 w-full">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Input placeholder={data.title} onChange={(e) => setForm({...form, title: e.target.value})} value={form.title}/>
                </div>
