    ...
    const handleSubmit = async() => {
        try {
            ...
            if(image && file && title) {
                ...
                const res = await axios.post(`${process.env.NEXT_PUBLIC_URL}magazine/create`, formData,{
                headers: {
                ...
                if(res.data.error) {
                    ...
                    setLoading(false)
                    toast({title : 'Sayı resmi, adı ve dosyası zorunludur'})
            }
        } catch (error: any) {
            setLoading(false)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
            <Button onClick={() => setOpen(true)}>Sayı Ekle</Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-[825px]">
            <DialogHeader>
                <DialogTitle>Sayı Ekle</DialogTitle>
            </DialogHeader>
            <div className="flex">
                ...
                <div className="flex flex-col gap-3 w-full">
                    <div className="flex flex-col gap-2">
                        ...
                        <Input placeholder="Bir şeyler yazın..." onChange={(e) => setTitle(e.target.value)} value={title}/>
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label>
                            Sayı Açıklaması
